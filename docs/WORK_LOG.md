...
## 2026-09-24 - character presentation audit

- Reviewed additional CMU boxing and Shotokan ready-pose candidates in a close renderer. Their moving feet and awkward upper-body retargeting made them unsuitable as a replacement idle. Rejected hair variants from the source character after close review; they read as a helmet or a hard-edged cap and are not used in the game.
- Applied the source character's CC0 sclera and iris maps to the head's eye surfaces, muted the unused clear cornea shell, disabled self-shadowing on eyeballs, and preserved the sclera's alpha window through appearance changes. This replaces the glowing white eyes with visible brown irises. Added brighter arena fill, adjusted clothing colors and curled fingers for a tighter guard. Added a close camera capture for visual inspection and made the source-pose viewer accept either study animation name.
- The close capture still shows a prototype-quality face, hands and guard. This is a limited readability pass; replacing the character/rig and authoring paired fighting performances remain necessary to approach the owner's references.
